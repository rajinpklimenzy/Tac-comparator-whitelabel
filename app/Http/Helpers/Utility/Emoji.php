...
class Emoji
{
    ...
    private static function convertEmoji($text, $operation = 'encode')
    {
        if (strtolower($operation) == "encode") {
            ...
            return preg_replace_callback('/(\\\u[0-9a-f]{4})+/', array('self', "callbackDecode"), $text);
        }
    }
    private static function callbackEncode($match)
    {
        return str_replace(array('[', ']', '"'), '', json_encode($match));
    }
    private static function callbackDecode($text)
    {
        if (!$text)
            return '';
        $text = $text[0];
