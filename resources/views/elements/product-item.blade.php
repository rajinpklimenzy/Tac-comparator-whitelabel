            ...
            @endphp
            @endforeach
            <div class="section-3-sec @if($min==$product['id']) featured @endif" id="{{$product['id']}}" data-target='el-2'>
            ...
            @if($p_type=='pack' && $packType=='gas' )
            @php $per1=100-(($discAmount/$price['totals']['month']['excl_promo'])*100) @endphp
            <li>
            ...
            @php $per1=100-(round((round($discAmount)/12)*100/round($price['totals']['month']['excl_promo']))*100) @endphp
            <li>
            <p class=" euro years "><b>&#8364; </i>@php $prc=number_format($price['totals']['year']['excl_promo']/100,2,',', '.');  $sp_price=preg_split("/,/",$prc)  @endphp {{$sp_price[0]}}</b><sup>,{{$sp_price[1]}}</sup</p><p class="@if($per1!=0) red @endif euro month"  style="display:none;"><b>&#8364;</i>@php $prc=number_format(($discAmount/12),2,',', '.');  $sp_price=preg_split("/,/",$prc)  @endphp {{$sp_price[0]}}</b><sup>,{{$sp_price[1]}}</sup></p>
            </li>
            @endif
            @endif
            @if($p_type=='pack' && $packType!='electricity' && $packType!='gas')
            ...
            <div class="tab">
                  ...
                  <button class="tablinks discountact" onclick="openCity(event, 'discount{{$product['id']}}')">@lang('home.Discounts')</button>
                   <button class="tablinks" onclick="openCity(event, 'history-details{{$product['id']}}')">@lang('home.history')</button>
                        ...
                        @endif
                         ...
                         @else
                          ...
                          @if((Session::get('promo')=='true' && $parameters['values']['promo_discount']['promo']=='promo') || (Session::get('domi')=='true' && $parameters['values']['promo_discount']['discount_serviceLevelPayment']=='domi') || (Session::get('email')=='true' && $parameters['values']['promo_discount']['discount_serviceLevelInvoicing']=='email'))
                            ...
                            @if($totalElec>0)
                            @if($parameters['values']['comparison_type']=='pack' || $parameters['values']['comparison_type']=='electricity' )
                        ...
                        <div class="nav-link lst" aria-controls="contact" aria-selected="false">
                            <span class="main-sec">
                                <span class="main-sec-1">
                                    ...
                                    <p class="mode amt">
                                        @if($parameters['values']['comparison_type']=="electricity")<i class="fa fa-bolt"></i></p>@endif
                                        @if($parameters['values']['comparison_type']=="gas")<i class="fa fa-fire"></i></p>@endif
                                        @if($parameters['values']['comparison_type']=="pack")<i class="fa fa-bolt"></i> + <i class="fa fa-fire"></i></p>@endif
                                </span>
                            </span>
                        </div>
                    </li>
                ...
                <div class="duration-sec">
                    ...
                    <p class="dur-p">
                        ...
                        @if (Session::get('locale')=='fr')
                            <a class="more-info" href="https://www.veriftarif.be/faq-foire-aux-questions/les-particuliers-et-petits-consommateurs-professionnels-ne-paieront-plus-de-frais-de-resiliation-de-leurs-contrats-d-energie" target="_blank" >@lang('home.More_info')</a>
                        @else
                            <a class="more-info" href="https://www.tariefchecker.be/faq/geen-verbrekingsvergoedingen-meer-voor-particulieren-en-kmo-s" target="_blank">@lang('home.More_info')</a>
                        @endif
                    </p>
            ...
            <div id="history-details{{$product['id']}}" class="tabcontent">
                  <input type="hidden" class="scenario{{$product['id']}}">
